 ...
 */
function convertContainerToHostPath(containerPath: string, groupFolder: string): string {
  ...
  if (containerPath.startsWith('/workspace/')) {
    ...
    if (relative.startsWith('global/')) {
      return path.join(DATA_DIR, 'groups', 'global', relative.substring('global/'.length));
    }
 ...
 */
function convertHostToContainerPath(hostPath: string, groupFolder: string): string {
  const workspaceRoot = path.join(DATA_DIR, 'sessions', groupFolder, 'group');
...
async function processRequest(
  ...
  try {
    // Read request
    const request: IFlowRequest = JSON.parse(fs.readFileSync(requestFile, 'utf-8'));
 ...
 */
function convertResultPaths(result: ToolResult, groupFolder: string): ToolResult {
  ...
  if (typeof result.result === 'string') {
    ...
    converted.result = result.result.map(item => {
      if (typeof item === 'string' && (item.startsWith('/') || item.includes(path.sep))) {
    ...
    converted.result = JSON.parse(
      JSON.stringify(result.result, (key, value) => {
        if (typeof value === 'string' && (value.startsWith('/') || value.includes(path.sep))) {
...
function watchGroupRequests(groupFolder: string): void {
  const requestsDir = path.join(DATA_DIR, 'sessions', groupFolder, 'ipc', 'iflow-requests');
  const responsesDir = path.join(DATA_DIR, 'sessions', groupFolder, 'ipc', 'iflow-responses');
    ...
    try {
      ...
      for (const file of files) {
        ...
        processRequest(requestFile, responseFile, groupFolder).catch(err => {
          logger.error({ error: err, groupFolder }, 'Failed to process request');
        });
      }
